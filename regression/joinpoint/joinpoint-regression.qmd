...
sites <- c("All sites", "Head and Neck", "Trunk",
           "Arm (shoulder to finger)", "Leg (hip to toe)")

grid <- expand_grid(Race = races, Sex = sexes, Site = sites)
...
models <- grid |>
  mutate(
    Model = future_pmap(
      list(Race, Sex, Site),
      function(race, sex, site) {
        tryCatch(
          joinpoint_model(data_melanoma, race, sex, site),
  ...
  mutate(
    Num_Breakpoints = map_int(Model, "num_breakpoints"),
    Joinpoint_Years = map(Model, "joinpoint_years"),
    Segment_Years = map(Model, "segment_years"),
    APC = map(Model, "apc"),
    AAPC = map(Model, "aapc")
  )
...
plot_joinpoint_set <- function(model_objs, labels, cols,
  ...
  # Helper to plot multiple data frames
  make_series_df <- function(model_obj, label) {
    d <- model_obj$data
    year_grid <- seq(min(d$Year), max(d$Year), by = 1)
    fitted_rate <- predict(model_obj$fit,
                           newdata = data.frame(Year = year_grid),
                           type = "link") |> exp() * 100000
    ...
    bind_rows(observed_df, fitted_df)
  }

  # Perform the plotting
  plot_df <- bind_rows(
    Map(make_series_df, model_objs, labels)
  )
  ...
  ggplot(plot_df, aes(x = Year, y = Rate, group = Series)) +
    geom_line(
      data = filter(plot_df, Type == "Fitted"),
      aes(color = Series),
      linewidth = 1
    ) +
